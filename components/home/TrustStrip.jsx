...
export default function TrustStrip({ bgClassName = "bg-[#f4efe6]" }) {
  const trustItems = [
    {
      id: "yoga-alliance",
  ...
  return (
    <section className={`${bgClassName} py-4 sm:py-6 lg:py-8 border-b border-[#e3dac9] relative z-20 font-figtree`}>
      <Container>
        {/* ========================================================= */}
        {/* 1. PC & TABLET VIEW: Perfectly Aligned 4-Pillar Grid (md & lg) */}
        {/* ========================================================= */}
        <div className="hidden md:block bg-[#fdfbf7] rounded-2xl lg:rounded-3xl border border-[#e3dac9] shadow-[0_4px_24px_rgba(0,0,0,0.03)] p-4 lg:p-5">
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-[#e3dac9]">
            {trustItems.map((item) => {
              const Content = (
                <div className="flex items-center gap-3.5 px-4 lg:px-5 py-2 group cursor-pointer transition-all duration-200">
                  ...
                  <div className="w-12 h-12 lg:w-13 lg:h-13 rounded-2xl bg-[#f4efe6] border border-[#e3dac9] group-hover:border-[#1c3b2b] group-hover:scale-105 transition-all duration-300 flex items-center justify-center shrink-0 p-2 shadow-2xs">
                    <Image
                      src={item.badge}
                      alt={item.alt}
                      width={40}
                  ...
                  <div className="flex flex-col justify-center min-w-0 flex-grow">
                    ...
                    <div className="h-5 flex items-center gap-1.5 overflow-hidden">
                      <span className="font-belleza text-[15px] lg:text-base text-[#1e2422] group-hover:text-[#1c3b2b] transition-colors truncate leading-none">
                        {item.title}
                      </span>
                      {item.url && (
                    ...
                    <div className="h-5 flex items-center gap-1 overflow-hidden my-0.5">
                      {item.hasStars ? (
                        <>
                          <div className="flex items-center gap-0.5 text-[#1c3b2b] shrink-0">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                        ...
                        <span className="text-[11px] font-bold text-[#1c3b2b] tracking-wide leading-none truncate">
                          {item.highlight}
                        </span>
                      )}
                    </div>
        ...
        <div className="block md:hidden">
          <div
            ...
            className="relative bg-[#fdfbf7] rounded-2xl border border-[#e3dac9] shadow-xs p-4 overflow-hidden"
          >
            ...
            {(() => {
              ...
              const cardContent = (
                <div className="flex items-center justify-center gap-3.5 text-left px-6 py-1">
                  {/* Badge */}
                  <div className="w-13 h-13 rounded-2xl bg-[#f4efe6] border border-[#1c3b2b]/40 flex items-center justify-center shrink-0 p-2 shadow-2xs">
                    <Image
                      src={active.badge}
                      alt={active.alt}
                      width={42}
                  ...
                  <div className="flex flex-col justify-center min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      {active.url && (
                        <ExternalLink className="w-3 h-3 text-[#1c3b2b]" />
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 my-1">
                      {active.hasStars ? (
                        <div className="flex items-center gap-1">
                          <div className="flex items-center gap-0.5 text-[#1c3b2b]">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                                className="w-3 h-3 fill-[#1c3b2b] text-[#1c3b2b]"
            ...
            <button
              ...
              className="absolute left-1.5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-[#fdfbf7] border border-[#e3dac9] flex items-center justify-center text-stone-600 shadow-xs hover:text-[#1c3b2b] transition-colors"
            >
            ...
            <button
              ...
              className="absolute right-1.5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-[#fdfbf7] border border-[#e3dac9] flex items-center justify-center text-stone-600 shadow-xs hover:text-[#1c3b2b] transition-colors"
            >
              <ChevronRight className="w-4 h-4" />
            </button>

            {/* Pagination Dots */}
            <div className="flex items-center justify-center gap-1.5 mt-3 pt-2 border-t border-[#e3dac9]/50">
              {trustItems.map((_, i) => (
                <button
                  key={i}
                  type="button"
                  ...
                  className={`transition-all duration-300 rounded-full ${
                    currentIndex === i
                      ? "w-5 h-1.5 bg-[#1c3b2b]"
                      : "w-1.5 h-1.5 bg-stone-300 hover:bg-[#1c3b2b]"
                  }`}
                />
              ))}
            </div>
