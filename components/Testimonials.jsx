...
import { Star, ChevronLeft, ChevronRight, Quote, ArrowUpRight, CheckCircle2 } from "lucide-react";
...
export default function Testimonials({ reviews = reviewsData }) {
  ...
  const visibleReviews = filteredReviews.slice(startIndex, startIndex + 3);
  ...
  return (
    ...
    >
      <Container>
        {/* Top Row: Section Heading on Left | Official Google & TripAdvisor Verification Badge on Right */}
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-5 mb-8 lg:mb-10">
          {/* Left: Eyebrow + Belleza Heading + Fortune Parade accent */}
          <div className="max-w-2xl">
            <span className="font-figtree text-[11px] sm:text-xs font-semibold tracking-[0.2em] uppercase text-[#f22924] block mb-3">
              Alumni Experiences &amp; Transformations
            </span>
            ...
            <p className="font-fortune text-lg sm:text-xl text-[#f22924]">
              Authentic Stories of Spiritual Growth in Rishikesh
            </p>
          </div>

          {/* Right: Live Google Rating Card (4.9 Stars • 48 Google Reviews) */}
          <div className="shrink-0 flex flex-col sm:flex-row gap-2.5">
            <a
              href={GOOGLE_SEARCH_URL}
              target="_blank"
              rel="noopener noreferrer"
              className="group inline-flex items-center gap-3 px-4 py-2.5 rounded-xl bg-white border border-[#EBE5D8] hover:border-[#f22924]/40 shadow-xs hover:shadow-md transition-all duration-300"
              title="View all 48 reviews on Google for Siddhant School of Yoga"
            >
              {/* Google Official Vector Logo */}
              <div className="shrink-0 flex items-center justify-center w-7 h-7 rounded-full bg-[#faf6ee] p-1 border border-stone-200">
                <Image
                  src="/logo/google.svg"
                  alt="Google"
                  width={24}
              ...
              <div className="flex flex-col text-left">
                <div className="flex items-center gap-1">
                  <span className="text-lg font-extrabold text-[#1F1E1D] leading-none font-figtree">
                    4.9
                  </span>
                  <div className="flex text-[#F5B860]">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                ...
                <div className="flex items-center gap-1 text-[10.5px] text-stone-600 font-semibold font-figtree mt-0.5">
                  <span>48 Google reviews</span>
                  <ArrowUpRight className="w-3 h-3 text-[#f22924] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                </div>
              </div>
            </a>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group hidden sm:inline-flex items-center gap-3 px-4 py-2.5 rounded-xl bg-white border border-[#EBE5D8] hover:border-[#00B386]/40 shadow-xs hover:shadow-md transition-all duration-300"
              title="View TripAdvisor ratings for Siddhant School of Yoga"
            >
              <div className="shrink-0 flex items-center justify-center w-7 h-7 rounded-full bg-[#e8f7f2] p-1 border border-[#00B386]/30">
                <Image
              ...
              <div className="flex flex-col text-left">
                <span className="text-[10px] uppercase tracking-wider text-[#023D38] font-bold leading-none mb-0.5 font-figtree">
                  TripAdvisor
                </span>
                <span className="text-[11px] font-semibold text-stone-700 font-figtree">
                  5.0★ Recommended
                </span>
              </div>
            </a>
        ...
        <div className="relative px-0 sm:px-5">
          ...
          {totalReviews > 3 && (
            <button
              ...
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-2 sm:-translate-x-4 w-9 h-9 rounded-full bg-white border border-[#EBE5D8] shadow-md hover:shadow-lg flex items-center justify-center text-[#55564c] hover:text-[#f22924] hover:border-[#f22924] transition-all z-20"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {visibleReviews.map((card) => (
              <div
                key={card.id}
                className="bg-white rounded-2xl p-4 sm:p-5 border border-[#EBE5D8] shadow-xs hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  {/* Top Row: Avatar + Reviewer Name + Course + Source Icon */}
                  <div className="flex items-center justify-between gap-2.5 mb-3">
                    <div className="flex items-center gap-2.5">
                      <div className="relative w-10 h-10 rounded-full overflow-hidden border border-[#f22924]/30 shrink-0 bg-[#faf6ee]">
                        <Image
                          src={card.avatar}
                          alt={card.name}
                          fill
                      ...
                      <div>
                        ...
                        <span className="text-[11px] text-[#f22924] font-medium block font-figtree">
                    ...
                    {/* Source Icon badge */}
                    <div className="shrink-0 flex items-center justify-center bg-[#FAF6EE] rounded-full p-1 border border-[#EBE5D8]">
                      {card.isTripAdvisor ? (
                        <Image
                          src="/logo/tripadvisor.svg"
                          alt="TripAdvisor"
                  ...
                  {/* 5 Filled Gold Stars */}
                  <div className="flex items-center gap-1 text-[#F5B860] mb-2">
                    {[...Array(card.rating)].map((_, idx) => (
                      <Star
                        key={idx}
                        className="w-3.5 h-3.5 fill-[#F5B860] text-[#F5B860]"
                      />
                    ))}
                  ...
                  {/* Highlight pill */}
                  {card.highlight && (
                    <div className="inline-block px-2 py-0.5 rounded-md bg-[#FAF6EE] border border-[#EBE5D8] text-[10.5px] font-semibold text-[#f22924] mb-2 font-figtree line-clamp-1">
                  ...
                  <div className="relative">
                    <Quote className="w-4 h-4 text-[#f22924]/20 mb-1" />
                    <p className="text-xs text-[#44453c] font-figtree leading-relaxed italic line-clamp-4 min-h-[64px]">
                      &ldquo;{card.text}&rdquo;
                    </p>
                  </div>
                </div>

                <div className="mt-3.5 pt-2.5 border-t border-[#EBE5D8]/80 flex items-center justify-between text-[11px] text-[#77786e] font-figtree">
                  <span className="font-medium text-stone-600">
                    {card.source}
                  </span>
                  <span className="text-[#3F6253] font-semibold flex items-center gap-1 text-[10.5px]">
                    <CheckCircle2 className="w-3 h-3 text-[#3F6253]" /> Verified Alumnus
                  </span>
                </div>
              </div>
            ))}
          ...
          {totalReviews > 3 && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-2 sm:translate-x-4 w-9 h-9 rounded-full bg-white border border-[#EBE5D8] shadow-md hover:shadow-lg flex items-center justify-center text-[#55564c] hover:text-[#f22924] hover:border-[#f22924] transition-all z-20"
            >
            ...
            {[...Array(maxIndex + 1)].map((_, i) => (
              <button
                ...
                className={`transition-all duration-300 ${
                  startIndex === i
                    ? "w-6 h-2 rounded-full bg-[#f22924]"
                    : "w-2 h-2 rounded-full bg-[#EBE5D8] hover:bg-[#F5B860]"
                }`}
              />
            ))}
          </div>
        ...
        <div className="mt-6 pt-4 border-t border-[#EBE5D8] flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs text-stone-600 font-figtree text-center sm:text-left">
            <span className="w-2 h-2 rounded-full bg-[#29A71A] animate-pulse" />
            <span>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-[#FAF6EE] hover:bg-[#f22924] text-stone-800 hover:text-white border border-[#EBE5D8] hover:border-[#f22924] text-xs font-bold font-figtree transition-all duration-200 shadow-2xs shrink-0"
          >
            <Image
              src="/logo/google.svg"
              alt="Google"
